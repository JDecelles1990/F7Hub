...
Revision persistence uses one connection and one BEGIN IMMEDIATE transaction: load current article → verify existence, DRAFT status and expected version → check unchanged content → conditional UPDATE by ID/version/DRAFT → append the new version snapshot → reload → COMMIT. A zero-row update is rejected. Snapshot/reload failures roll back all changes; retry adds exactly the next version. Article code and prior snapshots are never updated. No-change detection follows concurrency checks, so it cannot hide a stale editor. Save Revision reuses ServiceTaskRunner and refreshes only the Knowledge list/detail. No migration, framework or ownership changes. Restore/revert, unpublishing/unarchiving and richer relationship workflows remain deferred.

Slice 018 extends KnowledgeWorkspace → ArticleCategoryDialog → KnowledgeService.set_article_category → KnowledgeRepository.set_draft_category. Bootstrap explicitly shares CategoryRepository with KnowledgeService and TicketReferenceService. The Knowledge service reads active KNOWLEDGE options through that shared repository; its two-field KnowledgeCategoryOption avoids coupling to TicketReferenceOption. Current article queries resolve category_name by a nullable scalar lookup of the category primary key, without filtering inactive assigned names.

Slice 021 extends KnowledgeWorkspace → ArticleTagsDialog → KnowledgeService.set_article_tags → KnowledgeRepository.set_draft_tags. Bootstrap explicitly injects the shared global TagRepository. Tag reads and writes remain behind service/repository boundaries and reuse ServiceTaskRunner; GUI code contains no SQL. The repository owns exact-set replacement under BEGIN IMMEDIATE with DRAFT, version and updated-at guards. Current article records expose deterministic tag names for display; tags remain current metadata, not content history or FTS input.

Slice 022 extends the existing KnowledgeWorkspace → KnowledgeService.list_articles/search_articles → KnowledgeRepository read path with `tag_id`/`untagged_only`. KnowledgeService validates positive non-bool IDs, a boolean Untagged flag and mutually exclusive modes. Repository list/search queries add relational `EXISTS`/`NOT EXISTS` predicates against `knowledge_article_tags`, preserving one row per article, list ordering, MATCH and bm25 ordering. Global options reuse `TagRepository.list_tags()`; no filter repository/service or FTS tag content is added.

KnowledgeWorkspace owns a tag-reference runner independent of the category-reference runner. `filter_loading` is true while either runs, so existing MainWindow close protection needs no modification. Filter changes use the shared article runner and authoritative selection/detail reloads. Tag mutation, create/lifecycle reconciliation and explicit Ticket Open Article preserve or reset only the dimensions required for truthful reveal.

A BEGIN IMMEDIATE transaction validates authoritative DRAFT state, version_number and updated_at before category eligibility/no-op checks and a parameterized conditional metadata UPDATE. It reloads before commit. ServiceTaskRunner executes reference reads and writes; GUI code contains no SQL. No migration, taxonomy subsystem, category history or new architectural layer is added.

Slice 017 extends the same KnowledgeWorkspace → KnowledgeService.archive_article → KnowledgeRepository.archive_published_article path through ServiceTaskRunner. One BEGIN IMMEDIATE transaction checks authoritative existence/PUBLISHED/expected version, executes a parameterized conditional UPDATE of only status=ARCHIVED and updated_at, requires one affected row, reloads and commits. Original published_at, content/version/history and links remain unchanged. Missing/non-PUBLISHED/stale/persistence errors remain distinct and safely translated. Plain-text Cancel-default confirmation retains the reviewed token; no optimistic GUI mutation, new architecture layer, migration or FTS rewrite is introduced.

Slice 016 extends KnowledgeWorkspace → KnowledgeService.publish_article → KnowledgeRepository.publish_draft_article. The service validates positive integer article ID/version (rejecting bool), generates one UTC timestamp and translates missing/non-DRAFT/stale/persistence errors. The repository owns one BEGIN IMMEDIATE transaction: authoritative read → existence/DRAFT/version checks → parameterized conditional status/timestamp UPDATE → exactly-one-row check → reload → COMMIT. Failure rolls back. Publication changes only status, published_at and updated_at; content/version/history/relationships remain intact. ServiceTaskRunner executes the write asynchronously. No migration, new service/repository, lifecycle snapshot or FTS rebuild is introduced.
