...
# 4. Verified Current Repository Layout

The following is the tracked implementation layout verified on 2026-09-07.
It distinguishes implemented content from empty scaffold directories.
...
├── AutoHotkey\
│   ├── F7Hub.ahk
│   └── Launchers\F7HubLauncher.ahk
├── Database\
│   ├── Migrations\
│   │   └── 0001_core.sql … 0005_knowledge.sql
│   └── SQLite\F7Hub.db
├── Docs\
│   ├── 00_ProjectVision.md … 19_DocumentationIndex.md
│   ├── Archive\DocsOLD\
│   ├── Assets\Diagrams\
...
`Assets`, `Build`, `Config`, `Data`, `Installer`, `Logs`, `Plugins`,
`PowerShell`, `Releases`, and `zip` currently exist as empty scaffolding or
non-source containers. They are not active implementation areas. Root-level
project support files include `AGENTS.md`, `ROOT.md`, `README.md`,
`requirements.txt`, `Project.json`, and the project workspace/inventory files.
...
| Directory | Current content / owner |
|---|---|
| `Python\f7hub\` | Primary PySide6 application, services, repositories and SQLite infrastructure |
| `AutoHotkey\` | F7 launch/focus entry script and launcher helper |
| `Database\Migrations\` | Versioned SQLite schema migrations; `Database\SQLite\F7Hub.db` is a zero-byte legacy scaffold |
| `Docs\` | Canonical documentation, archived material, diagram assets and current-state snapshot |
| `Tests\` | AutoHotkey, database, GUI and integration tests |
| `Tools\Scripts\` | Project structure initialization script |

The remaining top-level directories are retained only as empty scaffolding for
future approved work. Their prospective ownership is described in the later
sections of this document; their presence does not mean the related subsystem
is implemented.
...
Repository inspection on 2026-09-07 confirmed:

```text
Tracked active directory layout: PASS
Canonical Docs/00–19 files: PASS
AutoHotkey F7 launch/focus entry point and launcher helper: VERIFIED
SQLite migrations through 0005_knowledge.sql: VERIFIED
Python application, services, repositories and GUI substructure: VERIFIED
Tests split into AutoHotkey, Database, GUI and Integration: PASS
Tools\Scripts\Initialize-F7HubStructure.ps1: PRESENT
Empty scaffold directories: PRESENT — not implementation evidence
```

`Python\f7hub\infrastructure\` contains the verified SQLite foundation;
`repositories\` and `services\` contain the ticket and knowledge boundaries;
`gui\` contains the current PySide6 workspaces and dialogs; and `app\`
centrally composes dependencies. `Python\f7hub\__main__.py` is the development
entry point. Tests remain separated under `Tests\AutoHotkey\`,
`Tests\Database\`, `Tests\GUI\` and `Tests\Integration\`.

`Database\SQLite\F7Hub.db` remains a zero-byte legacy scaffold and is not the
development database used by the automated tests. `Docs\Archive\DocsOLD\` is
historical only; its contents are not canonical sources of truth.
