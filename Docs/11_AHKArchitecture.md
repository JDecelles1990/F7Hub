...
`AutoHotkey/F7Hub.ahk`, `Hotkeys/F7HotkeyController.ahk`, `Launchers/F7HubLauncher.ahk` and `Helpers/MagneticWindowFollower.ahk` implement the F7 tap/hold shortcut. The entry point registers one shortcut-script instance and routes F7 down/up events through an explicit `IDLE` → `PENDING` → `HOLD_STARTING` → `HOLDING` state machine. The hold threshold is 180 ms. Releasing before the threshold performs TAP; after HOLD begins, release stops the follower and does not perform TAP. The follower also checks physical F7 state on every active tick so a missed key-up callback cannot leave movement running.

TAP uses `LaunchOrFocus()`: it finds or launches F7Hub, restores/shows it and requests foreground focus. HOLD uses `LaunchOrFind()`: it finds or launches the same validated F7Hub HWND and makes it visible without foreground activation. Hidden, minimized or maximized windows are normalized with `ShowWindow(SW_SHOWNOACTIVATE)` before movement. `Focus()` remains a separate TAP-only operation and retains its actionable Windows foreground-restriction error.

Existing windows are recognized by exact `F7Hub` title, Qt window class and Python process name, including hidden windows. That identity check is reused by the follower before start and on every tick so an invalid, closed or HWND-reused window stops the timer instead of moving another window. A pending process is retained after a 15-second startup timeout to prevent another launch on retry. This is a development launcher for one checkout: separately started copies with the same window identity are not distinguished. It is not an application-wide single-instance lock.

The follower has no inactive polling. HOLD starts one 16 ms timer with zero velocity; release, invalid identity, HWND disappearance or movement failure stops the timer and clears HWND, physical-key and velocity state. Each tick accelerates toward a cursor-offset target, applies damping, clamps vector speed, suppresses motion within a dead zone and skips `WinMove` when rounded X/Y would not change. The target is clamped to the work area of the monitor containing the cursor, including monitors with negative coordinates.

Actual displacement per tick is bounded by `MaxSpeed` (32 px) plus at most `sqrt(0.5)` px (approximately 0.71 px) from rounding both coordinates. Intermediate positions cross monitor gaps without snapping to the destination boundary. Each axis is restricted to the interval between its current position and target, preventing overshoot without increasing the bounded step. The dead zone applies only after containment; a short remaining approach outside the bounds finishes with a speed-limited step. If a window dimension exceeds the destination work-area dimension, that axis settles at the work-area start edge. The window is never resized.

One physical press permits only one HOLD attempt. Follower self-stop leaves the controller's HOLD consumed; the next repeated down changes `HOLDING` to `WAIT_RELEASE` without scheduling a timer. Launcher/lookup or follower-start failure also enters `WAIT_RELEASE` while F7 remains pressed. Repeats in `HOLD_STARTING`, `HOLDING` or `WAIT_RELEASE` cannot relaunch or restart movement. Key-up returns to `IDLE`, permitting a new press.

Start the shortcut by opening `AutoHotkey/F7Hub.ahk` with AutoHotkey v2. Use its tray menu to exit or reload. Login startup registration is not installed. The default app uses `Database/Dev/f7hub_dev.db`; the launcher class accepts explicit database and Python-runtime paths for isolated live tests.

```text
F7 tap controller/launcher workflow: PASS — automated live Windows test, 2026-09-16
F7 hold controller/launcher/follower workflow: PASS — automated live Windows test, 2026-09-16
Minimized non-activating restore: PASS — automated live Windows test, 2026-09-16
Three-monitor work-area movement: PASS — automated live Windows test, 2026-09-16
Physical global F7 keyboard validation: PASS — user-reported physical acceptance, 2026-09-21
Interactive temporal flicker observation: PASS — user-reported physical acceptance, 2026-09-21
```

`Tests/AutoHotkey/test_f7hub_launcher.ahk` uses a console-independent result file, a new isolated database path and an optional injected Python runtime. It checks missing runtime, repeated timeout without duplicate launch, cold launch, environment restoration, validated non-focusing lookup, controller TAP focus, HOLD without focus, release stop, minimized HOLD restore, rapid HOLD reuse, multi-monitor work-area movement, decoy rejection, shortcut registration and duplicate shortcut startup. Test-only key-state injection drives the controller because synthetic AutoHotkey input is not physical key state; production continues to use `GetKeyState("F7", "P")`. The test closes only the app and shortcut it starts and deletes its isolated database.

`Tests/AutoHotkey/test_magnetic_window_follower.ahk` reports through a temporary result file and covers invalid/validator rejection, duplicate-start prevention, start/stop lifecycle, physical-release fallback, HWND disappearance, velocity reset, dead-zone/max-speed helpers and multi-monitor coordinate lookup.
