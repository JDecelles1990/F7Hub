...
Validation and actual fresh regression totals are recorded in `Status/CURRENT_STATE.md`. Native Windows edit/input/visual checks passed at 1000×700 with synthetic SQLite. Restore/revert, unpublishing/unarchiving and richer relationship workflows remain deferred.
...
## Implemented Knowledge Status Filtering — Slice 020

KnowledgeService.list_articles/search_articles accept status=None and validate only DRAFT, PUBLISHED or ARCHIVED otherwise; lowercase, empty, unknown, integer and bool values are rejected before repository access. KnowledgeRepository composes this bound equality with the existing static category predicates. List ordering, literal FTS query generation, MATCH, bm25 ranking and current search-result records are unchanged.
...
## Implemented Any-Tag Filtering — Slice 024
...
KnowledgeWorkspace keeps category selection separate from executed search text. Clear Search retains the filter; explicit open-by-ID resets it; incompatible create results reveal under All. Category updates reconcile filtered list/search without optimistic mutation. A separate ServiceTaskRunner owns reference loading; signal-blocked population prevents duplicate requests. Its busy state disables only the selector while article reads remain independent; MainWindow checks it before closing. No new dependency or filtering subsystem. See Status/CURRENT_STATE.md for Slice 019 validation.

## Implemented Knowledge Category Boundary — Slice 018

KnowledgeService now requires explicit KnowledgeRepository and CategoryRepository dependencies. Bootstrap shares the taxonomy reader with TicketReferenceService; test constructors provide it explicitly. list_active_knowledge_categories returns immutable category_id/name options from list_categories(scope="KNOWLEDGE", active_only=True), with safe reference-load errors independent of article reads.

set_article_category validates positive integer article/version/category IDs excluding bool, allows None for removal, and requires a nonblank exact expected_updated_at token. One UTC clock read supplies a microsecond timestamp; if that clock is at or behind the reviewed timestamp, the new token advances one microsecond beyond it. The repository also refuses an unchanged token. Missing/non-DRAFT/stale conditions translate to KnowledgeCategoryConflictError; unavailable/no-op/persistence failures have safe KnowledgeCategoryError messages.
...
KnowledgeRepository.archive_published_article guards authoritative PUBLISHED state/version inside BEGIN IMMEDIATE, conditionally updates only status/updated_at with parameters, checks rowcount==1, reloads and commits. Original published_at and all content/history/link data remain intact; rollback covers reload failure. KnowledgeWorkspace retains the reviewed token across Cancel-default PlainText confirmation and runs the service through ServiceTaskRunner before refresh/reselect/reload. No separate lifecycle subsystem, migration, snapshot or FTS rewrite is introduced.
...
Slice 013 adds unlink_related_article(ticket_id, knowledge_article_id) to the existing service/repository. The service rejects invalid/nonpositive/out-of-range IDs and bool, accepts no relationship type or actor, and returns None on committed success. ArticleNotLinkedError translates to TicketKnowledgeNotLinkedError within the existing validation-error hierarchy; existing missing-entity and safe persistence errors are reused. The repository owns one BEGIN IMMEDIATE transaction with authoritative ticket/article/RELATED checks, an exact parameterized DELETE, rowcount == 1 and commit. Zero rows is not-linked; other unexpected counts, delete failures and commit failures roll back through the existing connection context.

Unlink Article uses the selected lightweight record for a plain-text QMessageBox, with Cancel as default/escape. A confirmation guard prevents reentry; captured ticket/article IDs are checked again after confirmation. Confirmed writes run through ServiceTaskRunner. Callbacks check the current ticket; success reuses refresh_links with committed-success feedback, while persistence failure preserves the selected row. No automatic DELETE retry occurs after a refresh failure. Both parent entities, other relationship types and ticket activity are untouched; read models, worker, workspace navigation and schema are unchanged. Focused and full results plus native Windows confirmation/unlink/relink evidence are recorded in Status/CURRENT_STATE.md.
