...
Slice 019 read-only category predicates: KnowledgeRepository.list_articles and search_articles accept keyword-only category_id=None and uncategorized_only=False. Defaults impose no restriction; uncategorized_only=True adds IS NULL; a specific ID adds a parameterized equality. Combining a specific ID with uncategorized_only is rejected. No status or active-category predicate hides current articles. Current names still resolve inactive references.

Search applies the predicate to ka.category_id beside MATCH, retaining bm25, updated_at DESC and ID DESC ordering. Normal lists retain updated_at DESC and ID DESC. Category names are not indexed or appended to MATCH. Filtering is SELECT only: no timestamps, versions, history, links or assignments change. All six migrations, schema and FTS triggers remain unchanged. Query-only connection tests, full dump comparison and isolated SQLite/FK/FTS integrity checks passed. See Status/CURRENT_STATE.md for validation.
