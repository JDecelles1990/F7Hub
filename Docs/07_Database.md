...
Slice 024 any-tag reads accept an optional validated tuple of distinct positive tag IDs, exclusive with specific-tag and Untagged modes. A bound `IN` list inside the existing correlated `EXISTS` matches one or more selected bridge rows without duplicating articles. Empty selection is All tags. Category/Status predicates and current FTS MATCH/bm25 ordering remain intact. Current article reads also return stable tag IDs beside display names for GUI reconciliation. Query-only tests, unchanged database dumps, integrity and foreign-key checks validate the SELECT-only boundary; no migration, schema, index or FTS object changed.

Slice 020 read-only status predicates: KnowledgeRepository.list_articles and search_articles accept keyword-only status=None in addition to Slice 019 category_id/uncategorized_only. None adds no lifecycle predicate; DRAFT, PUBLISHED or ARCHIVED adds a bound equality on status (ka.status for FTS joins). KnowledgeService validates the exact lifecycle domain before repository access. Category and status predicates compose independently using static internal fragments and parameterized external values.

Slice 022 read-only tag predicates add `tag_id=None` and `untagged_only=False`. A specific positive tag uses a bound `EXISTS` correlated on the authoritative current article ID; Slice 024 extends that predicate to an `IN` list for Any of selected tags. Untagged uses `NOT EXISTS` for any bridge row. The modes are mutually exclusive. This avoids duplicate result rows for multi-tag articles and composes with category/status in both normal lists and FTS joins without changing MATCH, indexed columns, ranking or triggers. Filtering writes nothing. The existing `(tag_id, knowledge_article_id)` index and bridge primary key are reused; all six migrations and the schema remain unchanged.

Search keeps MATCH unchanged and applies category/status relational predicates in the authoritative current-article join, retaining bm25, updated_at DESC and ID DESC ordering. Normal lists retain updated_at DESC and ID DESC. Neither filter is placed in MATCH or indexed content. Filtering is SELECT only: lifecycle, category, content, timestamps, versions/history, links and FTS content do not change. Existing status indexes are reused; all six migrations, schema and FTS triggers remain unchanged. See Status/CURRENT_STATE.md for validation.
...
No archived_at field, migration or new schema object is added; 0001–0006 remain unchanged. No content snapshot, version increment or FTS rewrite occurs. Search joins current ARCHIVED status from the relational row. Isolated validation checks schema identity, six migration records, integrity_check=ok, zero foreign-key violations and external-content FTS integrity. Unarchive, Unpublish and DRAFT archive remain unimplemented.
