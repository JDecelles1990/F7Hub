...
class TicketKnowledgeFlowTests(unittest.TestCase):
    ...
    def test_filter_lifecycle_and_ticket_open_outside_filter(self):
        ...
        self.confirm_publish(workspace)
        self.assertEqual(workspace.category_filter.currentData(), 11)
        self.assertEqual(workspace.status_filter.currentText(), 'All statuses')
        self.assertEqual(workspace.article.status, 'PUBLISHED')
        workspace.status_filter.setCurrentIndex(workspace.status_filter.findData('PUBLISHED'))
        self.wait_idle()
        self.confirm_archive(workspace)
        self.assertEqual(workspace.category_filter.currentData(), 11)
        self.assertEqual(workspace.status_filter.currentText(), 'All statuses')
        self.assertEqual(workspace.article.status, 'ARCHIVED')
        workspace.status_filter.setCurrentIndex(workspace.status_filter.findData('ARCHIVED'))
        self.wait_idle()
        ...
        self.assertEqual([v.version_number for v in viewer.versions], [2, 1])
        viewer.close()
        # The linked archived article is outside both current filter dimensions.
